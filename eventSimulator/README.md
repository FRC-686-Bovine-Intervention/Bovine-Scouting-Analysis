...
The recorder exposes `GET http://127.0.0.1:8788/status` and `POST /control/start`, `/control/stop`, and `/control/poll`. Configuration may be supplied through `EVENT_RECORDER_CONFIG` as a non-secret JSON file; keep `TBA_AUTH_KEY` in the environment. Use `node eventSimulator/recording-tools.mjs validate recordings/2026chcmp`, `inspect`, or `export <source> <destination>` for safe verification and intentional sharing.
