...
assert.equal((await get("/state")).currentMatch, "Semis 13-1");
const playoffMatches = await get("/api/tba/event/2026evsim/matches");
assert.equal(playoffMatches.filter((match) => match.comp_level === "sf" && match.alliances.red.score >= 0).length, 13);
assert.equal(playoffMatches.filter((match) => match.comp_level === "f" && match.alliances.red.score >= 0).length, 0);
